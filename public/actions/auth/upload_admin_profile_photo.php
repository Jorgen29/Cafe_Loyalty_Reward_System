 ...
 * - ensures `image_path` column exists on `user` table (adds if missing)
 * - updates `user.image_path` WHERE user_id = $_SESSION['user_id']
...
// Update DB (user.image_path) if possible
// db_config.php lives in the same `auth` directory
require_once __DIR__ . '/db_config.php';

$userId = $_SESSION['user_id'];

// Check if user table has image_path column
$hasColumn = false;
$colStmt = $conn->prepare("SHOW COLUMNS FROM user LIKE 'image_path'");
if ($colStmt) {
    $colStmt->execute();
...
if (!$hasColumn) {
    // Try to add column
    $alter = "ALTER TABLE user ADD COLUMN image_path VARCHAR(255) DEFAULT NULL";
    $conn->query($alter);
}

// Update user record
$update = $conn->prepare("UPDATE user SET image_path = ? WHERE user_id = ?");
if ($update) {
    $update->bind_param('si', $relative_path, $userId);
    if (!$update->execute()) {
        // Not fatal — return success for file save but warn
        error_log('Failed to update user image_path: ' . $conn->error);
