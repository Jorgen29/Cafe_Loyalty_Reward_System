<?php
/**
 * Menu Page Assets Save Handler
 * Saves menu page content and images to menu_page_assets table
 */
...
if (isset($_FILES['cover_image_file']) && $_FILES['cover_image_file']['error'] === UPLOAD_ERR_OK) {
    $cover_image = uploadImage($_FILES['cover_image_file'], 'menu_page');
    if (!$cover_image) {
...
// Check if record exists for Menu category
$checkQuery = $conn->prepare("SELECT h_assets_id, cover_image FROM home_page_assets WHERE category = 'Menu' LIMIT 1");
if (!$checkQuery) {
    http_response_code(500);
...
if ($exists) {
    ...
    $updateQuery = $conn->prepare("UPDATE home_page_assets SET cover_text = ?, cover_image = ? WHERE category = 'Menu'");
    if (!$updateQuery) {
        http_response_code(500);
    ...
    if (!$result) {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to update menu page assets']);
        exit;
    }

    $updateQuery->close();
    http_response_code(200);
    echo json_encode(['success' => true, 'message' => 'Menu page updated successfully']);
} else {
    // Insert new record
    ...
    if (!$result) {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to create menu page assets']);
        exit;
    }

    $insertQuery->close();
    http_response_code(201);
    echo json_encode(['success' => true, 'message' => 'Menu page created successfully']);
}
...
function uploadImage($file, $page_type) {
    ...
    $filename = 'menu_' . time() . '_' . uniqid() . '.' . $file_ext;
