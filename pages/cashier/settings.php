...
$cashierData = [];
$cashierName = htmlspecialchars($_SESSION['first_name'] . ' ' . $_SESSION['last_name']);
$userEmail = htmlspecialchars($_SESSION['email']);

if (isset($_SESSION['user_id'])) {
    $uid = intval($_SESSION['user_id']);
    $stmt = $conn->prepare("SELECT cashier_id, first_name, last_name, store_id FROM cashier WHERE user_id = ? LIMIT 1");
    if ($stmt) {
        ...
        (function() {
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    fetch('../../public/actions/auth/update_cashier_profile.php', {
                        method: 'POST',
                        body: fd
